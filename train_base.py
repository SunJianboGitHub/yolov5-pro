...
if __name__ == "__main__":
    ...
    # 训练配置文件，根据数据集必须修改的
    parser.add_argument("--cfg", type=str, default="/workspace/yolov5-pro/models/yolov5s.yaml", help="mode.yaml path.")                         # 模型配置文件, 主要修改检测类别数、anchors
    parser.add_argument('--data', type=str, default='/workspace/yolov5-pro/data/config/excavator2023.yaml', help='data.yaml path')                    # 数据集配置文件, 主要修改各个数据集路径
    parser.add_argument('--hyps', type=str, default='/workspace/yolov5-pro/data/hyps/hyp.excavator2023-prune.yaml', help='hyp.yaml path')             # 超参数配置文件, 主要修改学习率、损失函数权重
    parser.add_argument("--prefix", type=str, default="Excavator2023", help="Prefix is used mark datasets.")                                          # 前缀字符串, 主要修改为数据集名称
    
    # 训练权重加载与结果保存参数,必须修改
    parser.add_argument("--weights", type=str, default="/workspace/yolov5-pro/weights/best.pt", help="load pretrained weights.")                      # 预训练权重的路径
    parser.add_argument('--resume', type=str, default="/workspace/yolov5-pro/runs/train/excavator2023-exp1/weights/best.pt", help='resume checkpoints.')                                                           # 检查点文件的路径
    parser.add_argument("--save_dir", type=str, default="/workspace/yolov5-pro/runs/train/excavator2023-exp1", help="Save weights and logs.")                 # 日志文件的保存路径
