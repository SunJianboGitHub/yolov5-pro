...
class OptimizerBN:
    def __init__(self, model, sr=0.001, epochs=100):
        ...
        self.ignore_bn_list = []                                # 不需要进行稀疏训练的层,通常是bottleneck层
    ...
    def update_bn(self, epoch):
        ...
        for k, m in self.model.named_modules():
            if isinstance(m, Bottleneck):
                ...
                # 这里所说的C3是带跳跃链接的C3
                if m.add:                                                           # 存在跳跃链接
                    self.ignore_bn_list.append(k.rsplit(".", 2)[0] + "cv1.bn")              # C3网络层中的cv1卷积的BN不正则化
                    self.ignore_bn_list.append(k + ".cv1.bn")                            # BottleNect中的cv1卷积的BN
                    self.ignore_bn_list.append(k + ".cv2.bn")                            # BottleNect中的cv2卷积的BN
